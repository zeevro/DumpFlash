...
from FlashFile import *  # pylint: disable=unused-wildcard-import
from FlashDevice import *  # pylint: disable=unused-wildcard-import
from FlashUtil import *  # pylint: disable=unused-wildcard-import
    ...
    if options.filename:
        if options.add_oob:
            ...
            if options.remove_oob:
                print('Removing OOB from pages(0x%x - 0x%x) to %s' %
                      (start_page, end_page, output_filename))
            else:
                print('Copying OOB from pages(0x%x - 0x%x) to %s' %
                      (start_page, end_page, output_filename))

            flash_util.CopyPages(output_filename, start_page,
                                 end_page, options.remove_oob)
    else:
